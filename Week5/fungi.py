...
class Fungi(LivingThing):
    ...
    def __repr__(self) -> str:
        return f'Fungi (name={self.__name}, age={self._age}, energy={self._energy})'
    ...
    def absorb(self, amount: int) -> int:
        ...
        if potential_energy >= Fungi.MAX_ENERGY:
            self._energy = Fungi.MAX_ENERGY
            return potential_energy - Fungi.MAX_ENERGY
        else:
            self._energy = potential_energy
            return self._energy - Fungi.MAX_ENERGY

    def reproduce(self) -> bool:
        potential_energy = self._energy - Fungi.REPRODUCE_ENERGY

        if potential_energy >= Fungi.MIN_ENERGY:
            self._energy = potential_energy
            return True
        else:
            return False
